...
export const createGroupChat = async (groupRequest: GroupRequest): Promise<GroupConversationResponse> => {
    try {
        const response = await api.post<GroupConversationResponse>("/group", groupRequest);
        return response;
    } catch (error) {
...
export const fetchGroupConversations = async (): Promise<GroupConversationResponse[]> => {
    try {
        const response = await api.get<GroupConversationResponse[]>("/group-chats");
        return response;
    } catch (error) {
        if (error instanceof AxiosError) {
            console.error("Error fetching group conversations:", error.response?.data);
        } else {
...
export const updateGroupSettings = async (groupId: number, settings: GroupSettingsResponse): Promise<GroupSettingsResponse> => {
    try {
        const response = await api.put<GroupSettingsResponse>(`/${groupId}/settings`, settings);
        return response;
    } catch (error) {
        if (error instanceof AxiosError) {
...
export const removeParticipant = async (groupId: number, username: string): Promise<void> => {
    try {

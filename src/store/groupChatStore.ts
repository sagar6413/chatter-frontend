...
import { GroupConversationResponse, GroupRequest, GroupSettingsResponse } from "@/types/index";
...
interface GroupChatState {
  ...
  updateGroupSettings: (groupId: number, settings: GroupSettingsResponse) => Promise<void>;
...
export const useGroupChatStore = create<GroupChatState>()((set) => ({
  ...
  updateGroupSettings: async (groupId: number, settings: GroupSettingsResponse) => {
    try {
      const updatedGroupSettings = await updateGroupSettings(groupId, settings);
      set((state) => {
        const updatedGroupConversations = state.groupConversations.map((group) => {
      ...
      set((state) => {
        const updatedGroupConversations = state.groupConversations.map((group) => {
          if (group.conversationId === groupId) {
            return {
              ...group,
              participants: new Set([...group.participants].filter((participant) => participant.username !== username)),
            };
          }
          return group;
        });
